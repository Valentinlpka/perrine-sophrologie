...
export default function ArticleContent({ slug }: ArticleContentProps) {
    ...
    const getImageUrl = async (imagePath: string) => {
        const { data: { publicUrl } } = await supabase.storage.from('images').getPublicUrl(imagePath);
        return publicUrl;
    };
    ...
    useEffect(() => {
        const fetchArticle = async () => {
            ...
            const articleWithImageUrl = {
                ...data,
                image: data.image ? await getImageUrl(data.image) : '/placeholder-image.jpg'
            };

            setArticle(articleWithImageUrl);
            ...
            if (recents) {
                const recentsWithImageUrl = await Promise.all(
                    recents.map(async (article) => ({
                        ...article,
                        image: article.image ? await getImageUrl(article.image) : '/placeholder-image.jpg'
                    }))
                );
                setArticlesRecents(recentsWithImageUrl);
            }
            ...
            if (data.category) {
                ...
                if (lies) {
                    const liesWithImageUrl = await Promise.all(
                        lies.map(async (article) => ({
                            ...article,
                            image: article.image ? await getImageUrl(article.image) : '/placeholder-image.jpg'
                        }))
                    );
                    setArticlesLies(liesWithImageUrl);
                }
            }
